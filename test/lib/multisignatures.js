...
// Used for sending XAS to accounts
...
function sendXASfromMultisigAccount (amount, recipient) {
    node.api.put("/transactions")
        .set("Accept", "application/json")
        .send({
        ...
        .end(function (err, res) {
            // console.log(JSON.stringify(res.body));
            // console.log("Sending " + amount + " XAS to " + recipient);
            node.expect(res.body).to.have.property("success").to.be.true;
...
before(function (done) {
   for (var i = 0; i < (Accounts.length); i++) {
       if(Accounts[i] != null) {
           sendXAS(Accounts[i], i);
       }
   }
   sendXAS(MultisigAccount, null);
   done();
});
...
describe("PUT /multisignatures", function () {
    ...
    it("When account has 0 XAS. Should fail", function (done) {
        node.api.put("/multisignatures")
            .set("Accept", "application/json")
            .send({
...
describe("Sending another transaction", function () {
    ...
    it("When other transactions are still pending. Should be ok", function (done) {
        node.onNewBlock(function (err) {
            sendXASfromMultisigAccount(100000000, node.Gaccount.address);
            done();
        });
    });
});
