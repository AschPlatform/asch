...
Accounts.prototype.getAccount = (filter, fields, callback) => {
  let cb = callback
  if (typeof fields === 'function') {
    cb = fields
  }
  const { publicKey } = filter
  ...
  if (typeof fields === 'function') {
    return library.base.account.get(filter, done)
  }
  return library.base.account.get(filter, fields, done)
}

Accounts.prototype.getAccounts = (filter, fields, cb) => {
  library.base.account.getAll(filter, fields, cb)
}

Accounts.prototype.setAccountAndGet = (data, cb) => {
  ...
  return library.base.account.set(address, data, (err) => {
    if (err) {
      return cb(err)
    }
    return library.base.account.get({ address }, cb)
  })
}

Accounts.prototype.mergeAccountAndGet = (data, cb) => {
  let address = data.address || null
  if (address === null) {
    if (data.publicKey) {
  ...
  return library.base.account.merge(address, data, cb)
}
